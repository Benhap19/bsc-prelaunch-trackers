...
from bsc_radar_v3_core import (
    ...
    ProjectCandidate,
    format_pre_ca_alert,
    handle_signal as core_handle_signal,
    should_alert,
    extract_tickers,
    get_domain,
    normalize_url,
...
# Project keys already alerted during this process.
alerted_projects = set()
...
def search_x() -> int:
    ...
    for query in config.X_SEARCH_QUERIES:
        ...
        for tweet in data.get("data", []):
            candidate = process_x_post(
                tweet,
                users.get(str(tweet.get("author_id")), {}),
            )
            if candidate and candidate.website:
                inspect_website(candidate.website)
            processed += 1

    return processed


def handle_signal(signal: PreCASignal) -> Optional[ProjectCandidate]:
    """Process, persist, and alert on a normalized pre-CA signal."""
    candidate = core_handle_signal(signal)
    ...
    project = {
        ...
        "launch_date": "",
        "bsc_intent": 1 if candidate.network == "BSC" else 0,
        "prelaunch_score": candidate.score,
        "stage": candidate.stage,
        "source": candidate.source,
        "mentions": candidate.signal_count,
    ...
    key = f"{candidate.ticker.lower()}|{candidate.website.lower()}|{candidate.project_name.lower()}"

    if should_alert(candidate) and key not in alerted_projects:
        try:
            if send_telegram_message(format_pre_ca_alert(candidate)):
                alerted_projects.add(key)
        except Exception:
            log.exception("Failed to send pre-CA Telegram alert")
